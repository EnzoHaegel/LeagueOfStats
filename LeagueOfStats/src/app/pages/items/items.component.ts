...
export class ItemsComponent implements OnInit {
  ...
  public onSearch(): void {
    this.displayedItems = this.items.filter(item => item.name.toLowerCase().includes(this.searchedItem.value.toLowerCase()));
  }
  ...
  public onChangeEvent(event: any): void {
    if (event.target.value === '') {
      ...
      this.displayedItems = this.items.filter(item => item.name.toLowerCase().includes(event.target.value.toLowerCase()));
    }
  }
